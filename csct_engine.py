...
class CSCTConfig:
    ...
    gate_sup_weight: float = 0.1

    # SingleGate (channel Top-1 routing) hysteresis
    # Adds a bias toward keeping the previously selected channel.
    # 0.0 disables stickiness.
    channel_stickiness: float = 0.0
    
    # Multi-Gate parameters (time-series MultiGate)
    base_theta_freq: float = 0.5          # θ rhythm base frequency
    theta_mod_strength: float = 0.3       # θ modulation coefficient
    nmda_window_sharpness: float = 5.0    # NMDA window sigmoid sharpness
...
def extract_physical_features(y: torch.Tensor) -> torch.Tensor:
    ...
    feat = torch.cat([y, dy, d2y], dim=-1)  # [B, T, 3D]

    # Normalize per-feature dimension (robust enough for small D)
    with torch.no_grad():
        flat = feat.abs().reshape(-1, feat.shape[-1])
        scale = torch.std(flat, dim=0).clamp(min=1e-6) * 2.0  # [3D]
    feat = feat / scale.view(1, 1, -1)
    return torch.clamp(feat, -3.0, 3.0)
...
def straight_through_topk(
    ...
    logits_2d = logits.view(-1, K)
    
    hard_logits = logits_2d
    if use_gumbel:
        U = torch.rand_like(hard_logits).clamp_(eps, 1.0 - eps)
    ...
    hard.scatter_(dim=-1, index=topk_idx, src=torch.ones_like(topk_idx, dtype=logits_2d.dtype))
    hard = hard / float(k)
    
    soft = F.softmax(logits_2d / max(tau, eps), dim=-1)
...
class MultiGateClockBank_TimeSeries(nn.Module):
    ...
    def __init__(self, cfg: CSCTConfig):
        ...
        gate_in_dim = 6 * int(getattr(cfg, 'input_dim', 1))  # [feat_x(3D), feat_y(3D)]
        ...
        self.na_threshold_bias = nn.Parameter(torch.zeros(1))
        self.nmda_threshold_bias = nn.Parameter(torch.zeros(1))
        self.composition_sensitivity = nn.Parameter(torch.ones(1) * 0.2)  # From EX6
        self.gate_temp = nn.Parameter(torch.ones(1))
        
        # θ Phase Controller (from EX6)
        # Note: base_theta_freq should be in cfg, default 0.5
        base_freq = getattr(cfg, 'base_theta_freq', 0.5)
    ...
    def forward(self, feat: torch.Tensor, t_rel: float = 0.0):
        """
        Args:
            feat: Combined features [B, T, 6D] = [feat_x(3D), feat_y(3D)]
            t_rel: Relative time (0-1) within sequence
        Returns:
        ...
        safe_temp = self.gate_temp.clamp(min=0.3, max=2.0)
        safe_sens = self.composition_sensitivity.clamp(min=0.0, max=1.0)
        safe_theta_freq = self.theta_freq.clamp(min=0.1, max=2.0)
        
        # Input strength for composition sensitivity (from EX6)
        # For time-series, use feature magnitude
        strength = feat.norm(p=2, dim=-1, keepdim=True)  # [B, T, 1]
        
        # === Na⁺ Gate: Ultra-fast, parallel sparse activation (from EX6) ===
        na_logits = self.na_gate(feat)  # [B, T, n_clocks]
        na_logits = na_logits.clamp(min=-10.0, max=10.0)
        
        # Add Gumbel noise during training (from EX6)
        if self.training and self.cfg.use_gumbel:
            eps = 1e-9
            U = torch.rand_like(na_logits).clamp(eps, 1.0 - eps)
            gumbel = -torch.log(-torch.log(U))
        ...
        theta_mod = torch.sigmoid(self.theta_proj(feat))  # [B, T, 1]
        
        # θ rhythm: time-dependent phase (from EX6)
        t_tensor = torch.linspace(0, t_rel, T, device=device).view(1, T, 1)
        theta_phase = 2 * np.pi * safe_theta_freq * t_tensor * theta_mod
        theta_phase = theta_phase.clamp(min=-100.0, max=100.0)
        
        # === NMDA Gate: Phase-dependent integration window (from EX6) ===
        # NMDA opens during θ peak (mimics hippocampal LTP timing)
        nmda_window_sharpness = getattr(self.cfg, 'nmda_window_sharpness', 5.0)
        ...
        nmda_logits = nmda_logits.clamp(min=-10.0, max=10.0)
...
class CSCT_Engine(nn.Module):
    ...
    def __init__(self, cfg: CSCTConfig = None, use_multigate: bool = False, **kwargs):
        ...
        if use_multigate:
            ...
            # - If input_dim  > 1: choose ONE channel per timestep, then choose ONE clock
            D = int(getattr(cfg, "input_dim", 1))
            self.use_channel_top1 = (D > 1)

            if self.use_channel_top1:
                # Shared channel scorer: per-channel 6D features -> scalar score
                self.channel_gate_net = nn.Sequential(
                    nn.Linear(6, cfg.hidden_dim), nn.Tanh(),
        ...
        # Anchor is always 1-channel, so input features are always [y, dy, d²y] = 3 dims
        anchor_in_dim = 3  # Fixed: anchor features are always from 1-channel signal
        ...
        self.codebook = nn.Parameter(torch.randn(cfg.n_clocks, cfg.input_dim) * 0.5)
        # Bias is per-channel
        self.bias = nn.Parameter(torch.zeros(cfg.input_dim))
        self.log_beta = nn.Parameter(torch.tensor(np.log(cfg.beta)))
    
    def anneal_temperature(self):
        """Anneal temperature for gradual discretization."""
    ...
    def forward(self, x_target: torch.Tensor, y_anchor: torch.Tensor, 
        ...
        # Anchor features for anchor_gate_net are always computed from 1-channel anchor (ch0), i.e., [B,T,3].
        # Explicitly take only channel 0 to ensure input dim matches anchor_gate_net (which expects 3 dims)
        y_anchor_ch0 = y_anchor[..., :1] if y_anchor.shape[-1] > 1 else y_anchor  # [B, T, 1]
        feat_y_anchor = extract_physical_features(y_anchor_ch0)  # [B, T, 3]
        # For MultiGate feature concat, tile the 1-channel anchor across D channels so gate input dim matches 6*D.
        y_for_gate = y_anchor_ch0
        if y_for_gate.shape[-1] == 1 and x_target.shape[-1] > 1:
            y_for_gate = y_for_gate.expand(-1, -1, x_target.shape[-1])
        feat_y_gate = extract_physical_features(y_for_gate)  # [B, T, 3*D]
        feat = torch.cat([feat_x, feat_y_gate], dim=-1)      # [B, T, 6*D]
        ...
        if self.use_multigate:
            ...
            if getattr(self, "use_channel_top1", False):
                ...
                D = x_target.shape[-1]
                feat_x_ch = feat_x.view(B, T, D, 3)
                feat_y_ch = feat_y_gate.view(B, T, D, 3)
                feat_ch = torch.cat([feat_x_ch, feat_y_ch], dim=-1)  # [B,T,D,6]

                # Channel scores: [B,T,D]
                ch_scores = self.channel_gate_net(feat_ch).squeeze(-1)
                ...
                if stick > 0.0 and T > 1:
                    ...
                    # No hysteresis: plain Top-1 per timestep
                    if self.training:
                ...
                feat_sel = (ch_sel.unsqueeze(-1) * feat_ch).sum(dim=2)  # [B,T,6]
                logits = self.gate_net(feat_sel)  # [B,T,n_clocks]
            else:
                # input_dim == 1
                logits = self.gate_net(torch.cat([feat_x, feat_y_gate], dim=-1))  # [B,T,n_clocks]
        ...
        if bool(getattr(cfg, 'use_anchor_gate', True)) and gate_sup_w > 0.0:
            ...
            dy_a[:, 1:, 0] = dy_step.mean(dim=-1)                 # -> [B, T-1]
...
def create_timeseries_model(**kwargs):
    """Convenience factory for CSCT time-series engine."""
    cfg = CSCTConfig(**kwargs)
